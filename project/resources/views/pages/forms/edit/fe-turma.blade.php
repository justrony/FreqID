...
@section('title', 'Editar Turma - FreqID')

@section('content')
    <main class="col-md-9 ms-sm-auto col-lg-10 p-0">
        <header class="top-header p-3">
            <div class="d-flex justify-content-between align-items-center">
                <h5 class="m-0 text-secondary fw-bold">Editar Turma: {{ $schoolClass->name }}</h5>
        ...
        <div class="p-4">
            <div class="card border-0 shadow-sm">
                <div class="card-body">
                    ...
                    <form action="{{ route('turma.update', $schoolClass) }}" method="POST">
                        @csrf
                        @method('PUT')

                        <h6 class="border-bottom pb-2 mb-4 text-primary">Informações da Turma</h6>
                                ...
                                <select class="form-select @error('school_id') is-invalid @enderror"
                                    ...
                                    @foreach($schools as $school)
                                        <option value="{{ $school->id }}"
                                            {{ old('school_id', $schoolClass->school_id) == $school->id ? 'selected' : '' }}>
                                            {{ $school->name }}
                                        </option>
                                    @endforeach
                                </select>
